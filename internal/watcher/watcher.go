...
func (w *Watcher) getUserEmails(ctx context.Context) ([]string, error) {
	...
	users, err := w.store.GetUsersForFeed(ctx, feedID)
	if err != nil {
		return nil, err
	}
...
func (w *Watcher) handleFeedResponse(ctx context.Context, resp crawler.CrawlResponse) time.Duration {
	...
	if resp.Error != nil {
		...
		if err := w.store.SetFeedError(ctx, feedID, resp.StatusCode, snippet); err != nil {
		...
		if err := w.store.UpdateFeedBackoff(ctx, feedID, time.Now().Add(w.currentInterval)); err != nil {
	...
	feedID := w.feed.ID
	etag := w.feed.ETag
	lastMod := w.feed.LastModified
	w.mu.RUnlock()

	if err := w.store.UpdateFeedBackoff(ctx, feedID, time.Time{}); err != nil {
		w.logger.Error("failed to clear backoff state", "error", err)
	}

	// Clear error in DB on success
	if err := w.store.ClearFeedError(ctx, feedID); err != nil {
		w.logger.Error("failed to clear feed error in DB", "error", err)
	}

	if resp.StatusCode == http.StatusNotModified {
		w.logger.Debug("feed not modified since last poll")
		if err := w.store.UpdateFeedLastPoll(ctx, feedID, etag, lastMod); err != nil {
	...
	w.mu.Lock()
	w.cachedEmails = userEmails
	fullArticle := w.feed.FullArticle
	feedTitle := w.feed.Title
	w.mu.Unlock()
	...
	for _, itm := range feed.Items {
		...
		seen, err := w.store.IsSeen(ctx, feedID, guid)
		if err != nil {
		...
		if fullArticle && itm.Link != "" {
			...
			if err := w.store.MarkSeen(ctx, feedID, guid); err != nil {
...
func (w *Watcher) handleItemResponse(ctx context.Context, resp crawler.CrawlResponse) {
	...
	feedID := w.feed.ID
	feedTitle := w.feed.Title
	extStrategy := w.feed.ExtractionStrategy
	extConfig := w.feed.ExtractionConfig
	w.mu.RUnlock()
	...
	if resp.Error == nil {
		ext, extErr := extractor.New(extStrategy, extConfig)
		if extErr != nil {
	...
	subject, body := w.FormatItem(feedTitle, itm, extractedContent)
	...
	if err := w.store.MarkSeen(ctx, feedID, resp.ItemGUID); err != nil {
